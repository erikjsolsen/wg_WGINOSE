...
for (i in 1:length(area)){ 
  ...
  if(length(wntg)>0) {trans.std<-trans.std[,-c(wntg)]}
  rownames(trans.std)<-c(data[,1])
  ...
  #' ---------------------
  #' Perform PCA
  ts<-trans.std[2:length(trans.std)]
  data.pca <- prcomp(ts, cor = F)
  summary(data.pca)
  ...
  write.csv(loadings.order, file = paste("PCA_WGINOSE17_Varloadings_",area.names[i], area[i], ".csv", sep=""))
  ...
  pdf(paste("WGINOSE17_PCA_ordniation_", area.names[i], area[i], ".pdf", sep=""))
