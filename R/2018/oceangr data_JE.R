...
where<-"/Users/eriko/ownCloud/Research/WGINOSE/2018/data/20180406_WGINOSE_bottom_surface_temp_psal_nut/"
...
for(i in 1:length(filer.s)) {
  ...
  fil<-read.table(paste(where,filer.s[i],sep=''),skip=1,header=T,sep=',',fill=TRUE)#Had to add to not get error message##
...
for(i in 1:length(filer.b)) {
  ...
  fil<-read.table(paste(where,filer.b[i],sep=''),skip=1,header=T,sep=',',fill=TRUE)
...
write.table(xy,"/Users/eriko/ownCloud/Research/WGINOSE/2018/data/20180406_WGINOSE_bottom_surface_temp_psal_nut/oceanogr data 1983_2018.csv",row.names=F,quote=F,sep=',')
